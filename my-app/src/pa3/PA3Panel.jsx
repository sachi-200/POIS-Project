...
import {
  encCPA, decCPA, playCPAGameRound,
  runCPASimulation, demonstrateNonceReuseAttack,
} from "./crypto.js";
...
export default function PA3Panel() {
  ...
  function doEnc() {
    ...
    setEncResult(r); setDecResult(null); setDecInput(r.ciphertext);
  }
  function doDec() {
  ...
  function doEncryptChallenge() {
    ...
    setGameRound(round); setGuess(null); setShowResult(false);
  }
  function doGuess(g) {
    if (!gameRound || showResult) return;
    setGuess(g); setShowResult(true);
    ...
    <div style={{ border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", overflow: "hidden" }}>
      ...
      <div style={{ padding: "16px" }}>
        ...
        <div style={{ padding: "8px 14px", borderRadius: "var(--border-radius-md)", background: mc.bg, border: `0.5px solid ${mc.border}`, color: mc.text, fontSize: 12, marginBottom: 16 }}>
          {reuseNonce
            ? "Broken mode: r is always F_k(0) — same plaintext always produces the same ciphertext."
            : "Secure mode: r ← {0,1}ⁿ freshly each encryption. Advantage should converge to ≈ 0."}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 16, marginBottom: 20 }}>
          <div>
            ...
            <button onClick={doEnc} style={{ width: "100%", padding: "8px 14px", fontSize: 12, fontWeight: 500, border: "0.5px solid #1D9E75", borderRadius: "var(--border-radius-md)", background: "#E1F5EE", color: "#0F6E56", cursor: "pointer", fontFamily: "var(--font-sans)", marginBottom: 12 }}>Encrypt</button>
            {encResult && (
              <div>
          ...
          <div>
            ...
            <button onClick={doDec} style={{ width: "100%", padding: "8px 14px", fontSize: 12, fontWeight: 500, border: "0.5px solid #378ADD", borderRadius: "var(--border-radius-md)", background: "#E6F1FB", color: "#185FA5", cursor: "pointer", fontFamily: "var(--font-sans)", marginBottom: 12 }}>Decrypt</button>
            {decResult && (
        ...
        <div style={{ borderTop: "0.5px solid var(--color-border-tertiary)", paddingTop: 16, marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 10, marginBottom: 14 }}>
            {[{ label: "Rounds", val: rounds }, { label: "Correct", val: correct }, { label: "Advantage", val: advantage.toFixed(3) }, { label: "Target", val: reuseNonce ? "≈ 1.0" : "≤ 0.1" }].map((s, i) => (
          ...
          <div style={{ height: 8, borderRadius: 4, background: "var(--color-background-secondary)", overflow: "hidden", border: "0.5px solid var(--color-border-tertiary)", marginBottom: 14 }}>
            <div style={{ height: "100%", width: `${Math.min(advantage, 1) * 100}%`, background: reuseNonce ? "#E24B4A" : advantage <= 0.1 ? "#1D9E75" : "#BA7517", transition: "width 0.4s" }} />
          </div>
              ...
              {gameRound ? (
                <div>
                  <div style={{ padding: "10px 12px", background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", marginBottom: 10 }}>
                    ...
                    <div style={{ display: "flex", gap: 6 }}><span style={{ fontSize: 10, color: "var(--color-text-secondary)", minWidth: 12 }}>c:</span><span style={{ fontFamily: "var(--font-mono)", fontSize: 11, wordBreak: "break-all" }}>0x{gameRound.c}</span></div>
                  </div>
                  {!showResult ? (
                ...
                <div style={{ padding: "20px", textAlign: "center", fontSize: 12, color: "var(--color-text-secondary)", background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)" }}>Enter m₀, m₁ and click Encrypt.</div>
              )}
            </div>
          </div>
        ...
        {/* ── CPA simulation + nonce-reuse attack ── */}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 16 }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                ...
                <button onClick={() => setSimResult(runCPASimulation(keyHex, prfType, reuseNonce, 50))} style={{ padding: "6px 12px", fontSize: 11, fontWeight: 500, border: "0.5px solid #7F77DD", borderRadius: "var(--border-radius-md)", background: "#EEEDFE", color: "#3C3489", cursor: "pointer", fontFamily: "var(--font-sans)", whiteSpace: "nowrap" }}>Run sim</button>
              </div>
              {simResult && (
                <div>
                  ...
                  <div style={{ padding: "8px 12px", borderRadius: "var(--border-radius-md)", background: parseFloat(simResult.advantage) <= 0.15 && !reuseNonce ? "#E1F5EE" : reuseNonce && parseFloat(simResult.advantage) > 0.8 ? "#FCEBEB" : "#FAEEDA", border: `0.5px solid ${parseFloat(simResult.advantage) <= 0.15 && !reuseNonce ? "#1D9E75" : reuseNonce ? "#E24B4A" : "#BA7517"}`, fontSize: 12, color: parseFloat(simResult.advantage) <= 0.15 && !reuseNonce ? "#0F6E56" : reuseNonce ? "#A32D2D" : "#854F0B" }}>
                    {reuseNonce ? "broken mode: adversary wins trivially!" : parseFloat(simResult.advantage) <= 0.15 ? "secure mode: advantage ≈ 0 ✓" : "advantage non-trivial"}
                  </div>
                </div>
              )}
            </div>
            <div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                <SectionHeading>Nonce reuse attack demo</SectionHeading>
                <button onClick={() => setAttackResult(demonstrateNonceReuseAttack(keyHex, prfType))} style={{ padding: "6px 12px", fontSize: 11, fontWeight: 500, border: "0.5px solid #D85A30", borderRadius: "var(--border-radius-md)", background: "#FAECE7", color: "#993C1D", cursor: "pointer", fontFamily: "var(--font-sans)", whiteSpace: "nowrap" }}>Run attack</button>
              </div>
              <div style={{ fontSize: 12, color: "var(--color-text-secondary)", marginBottom: 10 }}>Enc(m) twice with reuse-r produces identical ciphertexts — trivially breaking IND-CPA.</div>
              {attackResult && (
                <div>
                  <div style={{ padding: "10px 12px", background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", marginBottom: 8 }}>
                    <div style={{ display: "flex", gap: 8, marginBottom: 4 }}><span style={{ fontSize: 10, color: "var(--color-text-secondary)", minWidth: 24 }}>m:</span><span style={{ fontFamily: "var(--font-mono)", fontSize: 11 }}>0x{attackResult.m}</span></div>
                    <div style={{ display: "flex", gap: 8, marginBottom: 4 }}><span style={{ fontSize: 10, color: "var(--color-text-secondary)", minWidth: 24 }}>C₁:</span><span style={{ fontFamily: "var(--font-mono)", fontSize: 11, wordBreak: "break-all" }}>{attackResult.ct1}</span></div>
                    <div style={{ display: "flex", gap: 8, marginBottom: 8 }}><span style={{ fontSize: 10, color: "var(--color-text-secondary)", minWidth: 24 }}>C₂:</span><span style={{ fontFamily: "var(--font-mono)", fontSize: 11, wordBreak: "break-all" }}>{attackResult.ct2}</span></div>
                  ...
                  {attackResult.detected && (
                    <div style={{ padding: "10px 14px", borderRadius: "var(--border-radius-md)", background: "#FCEBEB", border: "0.5px solid #E24B4A", color: "#A32D2D", fontSize: 12 }}>
                      Adversary queries Enc(m) twice and sees C₁ = C₂. IND-CPA is broken.
                    </div>
                  )}
                </div>
              )}
