...
// PRF-MAC · CBC-MAC · EUF-CMA game · MAC⇒PRF · Length-extension attack demo
// ═══════════════════════════════════════════════════════════════════════════════
...
function ResultBanner({ ok, okMsg, failMsg }) {
  return (
    <div style={{
      marginTop: 10, padding: "10px 12px", borderRadius: "var(--border-radius-md)", fontWeight: 500, fontSize: 12,
...
// ── CBC-MAC chain SVG visualiser ──────────────────────────────────────────────

function CBCChainViz({ steps }) {
  if (!steps || steps.length === 0) return null;
  const BOX_W = 72, BOX_H = 32, XOR_R = 14, GAP = 52;
  const totalW = (BOX_W + GAP) * steps.length + 80;
  const MID_Y  = 56;

  return (
    <svg width="100%" viewBox={`0 0 ${totalW} 130`} style={{ display: "block", fontFamily: "var(--font-mono)", overflow: "visible" }}>
      {/* IV box */}
      <rect x={0} y={MID_Y - BOX_H / 2} width={42} height={BOX_H} rx={4}
        fill="var(--color-background-secondary)" stroke="#D3D1C7" strokeWidth={0.5} />
      <text x={21} y={MID_Y + 5} textAnchor="middle" fontSize={9} fill="#888780">0^8</text>
      ...
      {steps.map((s, i) => {
        ...
        return (
          <g key={i}>
            ...
            {/* message block drop-in */}
            <rect x={xorX - 20} y={MID_Y - 56} width={40} height={24} rx={4}
              fill="#E1F5EE" stroke="#1D9E75" strokeWidth={0.8} />
            <text x={xorX} y={MID_Y - 41} textAnchor="middle" fontSize={8} fill="#0F6E56">M{i + 1}</text>
            <text x={xorX} y={MID_Y - 29} textAnchor="middle" fontSize={7} fill="#0F6E56">{s.block}</text>
            <line x1={xorX} y1={MID_Y - 32} x2={xorX} y2={MID_Y - XOR_R}
            ...
            <text x={fkX + BOX_W / 2} y={MID_Y - 3} textAnchor="middle" fontSize={9} fill="#185FA5">F_k</text>
            <text x={fkX + BOX_W / 2} y={MID_Y + 11} textAnchor="middle" fontSize={7.5} fill="#185FA5">
              0x{s.next.slice(0, 6)}
            </text>
          </g>
        );
      ...
      {(() => {
        ...
        return (
          <g>
            <line x1={lastFkX + BOX_W} y1={MID_Y} x2={lastFkX + BOX_W + 30} y2={MID_Y}
              stroke="#B5B1A8" strokeWidth={1} />
            <rect x={lastFkX + BOX_W + 30} y={MID_Y - 14} width={44} height={28} rx={4}
              fill="#EEEDFE" stroke="#7F77DD" strokeWidth={1} />
            <text x={lastFkX + BOX_W + 52} y={MID_Y + 5} textAnchor="middle" fontSize={9} fill="#3C3489">tag t</text>
          </g>
        );
      })()}
...
export default function PA5Panel() {
  ...
  const [pmMsg,        setPmMsg]        = useState("10110010");   // 8-bit binary
  ...
  const [hmacKey,    setHmacKey]    = useState("a3f2c1b8");
  const [hmacMsg,    setHmacMsg]    = useState("10110010");
  const [hmacError,  setHmacError]  = useState(null);
  const [hmacCalled, setHmacCalled] = useState(false);
  ...
  function doHmac() {
    setHmacCalled(true);
    try { hmac(hmacKey, hmacMsg); setHmacError(null); }
    catch (e) { setHmacError(e.message); }
  }
  ...
  return (
    <div style={{ border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", borderBottom: "0.5px solid var(--color-border-tertiary)", background: "var(--color-background-secondary)", overflowX: "auto" }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)} style={{
            ...
            border: "none",
            borderBottom: activeTab === t.id ? "2px solid #BA7517" : "2px solid transparent",
            background: "transparent", cursor: "pointer",
            color: activeTab === t.id ? "#7A4A00" : "var(--color-text-secondary)",
            fontFamily: "var(--font-sans)", whiteSpace: "nowrap",
          }}>{t.label}</button>
        ))}
      </div>
      ...
      <div style={{ padding: 16 }}>
        ...
        {activeTab === "prf-mac" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              ...
              {/* Sign */}
              <div>
                <div style={{ fontWeight: 500, fontSize: 12, marginBottom: 10 }}>Sign</div>
                <div style={{ marginBottom: 8 }}>
                  <FieldLabel>Key k (hex, 8 chars)</FieldLabel>
                  <TextInput value={pmKey} onChange={setPmKey} placeholder="a3f2c1b8" />
                </div>
                <div style={{ marginBottom: 12 }}>
                  {/* FIX: label correctly says binary */}
                  <FieldLabel>Message m (8-bit binary string ∈ &#x7B;0,1&#x7D;⁸)</FieldLabel>
                  <TextInput
                    value={pmMsg}
                    onChange={v => setPmMsg(v.replace(/[^01]/g, "").slice(0, 8))}
                    placeholder="e.g. 10110010"
                  />
                  <div style={{ fontSize: 10, color: "var(--color-text-secondary)", marginTop: 3 }}>
                    Normalised: {pmResult.m}
                  </div>
                </div>
                <div style={{ padding: "12px", background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)" }}>
                  ...
                  <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--color-text-secondary)", marginBottom: 4 }}>
                    F_{prfType}(k=0x{pmKey.slice(0, 6)}, m={pmResult.m})
                  </div>
              ...
              {/* Verify */}
              <div>
                <div style={{ fontWeight: 500, fontSize: 12, marginBottom: 10 }}>Verify</div>
                <div style={{ marginBottom: 8 }}>
                  <FieldLabel>Tag to verify (hex)</FieldLabel>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                  ...
                  <button onClick={() => { setPmVerifyTag(pmResult.tag); setPmVerifyRes(null); }}
                    style={btnStyle("#E6F1FB", "#B5D4F4", "#185FA5")}>Copy current tag</button>
                </div>
                {pmVerifyRes !== null && (
                  <ResultBanner
                    ok={pmVerifyRes}
                    okMsg="✓ Tag valid — Vrfy(k, m, t) = 1"
                    failMsg="✗ Tag invalid — Vrfy(k, m, t) = 0"
                  />
                )}
                <div style={{ marginTop: 12, padding: "10px 12px", background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", fontSize: 11, color: "var(--color-text-secondary)" }}>
                  Interface exported for PA#6: <span style={{ fontFamily: "var(--font-mono)", color: "#7A4A00" }}>Mac(k,m) → t</span> and <span style={{ fontFamily: "var(--font-mono)", color: "#7A4A00" }}>Vrfy(k,m,t) → bool</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === "cbc-mac" && (
          <div>
            <SectionHeading>Construction 2 — CBC-MAC: chain F_k over padded message blocks, output final F_k result</SectionHeading>
            <div style={{ display: "grid", gridTemplateColumns: "260px minmax(0,1fr)", gap: 16 }}>
              <div>
                <div style={{ marginBottom: 8 }}>
                  <FieldLabel>Key k (hex)</FieldLabel>
                  <TextInput value={cbcKey} onChange={setCbcKey} placeholder="a3f2c1b8" />
                </div>
                <div style={{ marginBottom: 12 }}>
                  {/* FIX: label correctly says binary */}
                  <FieldLabel>Message m (binary string, any length)</FieldLabel>
                  <TextInput
                    value={cbcMsg}
                    onChange={v => setCbcMsg(v.replace(/[^01]/g, ""))}
                    placeholder="e.g. 1011001011001010"
                  />
                </div>
                ...
                <div style={{ padding: "12px", background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", marginBottom: 10 }}>
                  <div style={{ fontSize: 10, color: "var(--color-text-secondary)", marginBottom: 6 }}>
                    Blocks after padding: {cbcResult.blocks.length}
                  </div>
                  {cbcResult.blocks.map((b, i) => (
                    <div key={i} style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--color-text-secondary)", marginBottom: 2 }}>
                      {/* FIX: blocks are binary — no 0x prefix */}
                      M{i + 1} = {b}
                    </div>
                  ))}
                ...
                <div style={{ marginBottom: 6 }}>
                  <FieldLabel>Tag to verify (hex)</FieldLabel>
                  <TextInput value={cbcVerify} onChange={setCbcVerify} placeholder="paste tag" />
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => setCbcVerRes(cbcMacVerify(cbcKey, cbcMsg, cbcVerify, prfType))}
                    style={btnStyle("#FEF3E2", "#E8B96A", "#7A4A00")}>Verify</button>
                  <button onClick={() => { setCbcVerify(cbcResult.tag); setCbcVerRes(null); }}
                    style={btnStyle("#E6F1FB", "#B5D4F4", "#185FA5")}>Copy tag</button>
                </div>
                {cbcVerRes !== null && (
                  <ResultBanner ok={cbcVerRes} okMsg="✓ Tag valid" failMsg="✗ Tag invalid" />
                )}
              </div>

              {/* Visualiser + step trace */}
              <div>
                <SectionHeading>CBC-MAC chain — {cbcResult.steps.length} block{cbcResult.steps.length !== 1 ? "s" : ""}</SectionHeading>
                <div style={{ padding: 10, background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", overflowX: "auto", marginBottom: 10 }}>
                  <CBCChainViz steps={cbcResult.steps} />
                </div>

                <SectionHeading>Step trace</SectionHeading>
                <div style={{ border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-md)", overflow: "hidden" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "44px 1fr 1fr 1fr 1fr", background: "var(--color-background-secondary)", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
                    {["#", "state_in (bin)", "block (bin)", "XOR (bin)", "F_k(·) (hex)"].map((h, i) => (
                  ...
                  {cbcResult.steps.map((s, i) => (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "44px 1fr 1fr 1fr 1fr", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
                      <div style={{ padding: "5px 8px", fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--color-text-secondary)" }}>{i + 1}</div>
                      <div style={{ padding: "5px 8px", fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--color-text-secondary)" }}>{s.state}</div>
                      <div style={{ padding: "5px 8px", fontFamily: "var(--font-mono)", fontSize: 10, color: "#0F6E56" }}>{s.block}</div>
                      <div style={{ padding: "5px 8px", fontFamily: "var(--font-mono)", fontSize: 10, color: "#854F0B" }}>{s.xored}</div>
                      <div style={{ padding: "5px 8px", fontFamily: "var(--font-mono)", fontSize: 10, color: "#3C3489" }}>0x{s.next}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {activeTab === "euf-cma" && (
          <div>
            <SectionHeading>EUF-CMA Forgery Game — hidden key k, up to 50 signed pairs</SectionHeading>
            <div style={{ display: "grid", gridTemplateColumns: "280px minmax(0,1fr)", gap: 16 }}>
              <div>
                <div style={{ padding: "10px 12px", background: "#FEF3E2", borderRadius: "var(--border-radius-md)", fontSize: 11, color: "#7A4A00", marginBottom: 12, border: "0.5px solid #E8B96A" }}>
                  Key k is hidden. You see (m_i, t_i) pairs signed under k.
                  Forge a valid tag on a <em>new</em> message not in the list.
                </div>
                ...
                {eufPairs && (
                  <>
                    <div style={{ marginBottom: 8 }}>
                      {/* FIX: label says binary, not hex */}
                      <FieldLabel>Forge message m* (8-bit binary — not in list)</FieldLabel>
                      <TextInput
                        value={forgeMsg}
                        onChange={v => setForgeMsg(v.replace(/[^01]/g, "").slice(0, 8))}
                        placeholder="e.g. 11001010"
                      />
                    </div>
                    <div style={{ marginBottom: 10 }}>
                      <FieldLabel>Forge tag t* (hex)</FieldLabel>
                      <TextInput value={forgeTag} onChange={setForgeTag} placeholder="your guessed tag…" />
                    </div>
                    <button onClick={doForgery} style={btnStyle("#FCEBEB", "#E24B4A", "#A32D2D", { width: "100%", marginBottom: 12 })}>
                      Submit forgery
                    </button>

                    {forgeResult && (
                      <div style={{
                        padding: "10px 12px", borderRadius: "var(--border-radius-md)", fontWeight: 500, fontSize: 12, marginBottom: 10,
                        background: forgeResult.accepted ? "#FCEBEB" : "#E1F5EE",
                        border: `0.5px solid ${forgeResult.accepted ? "#E24B4A" : "#1D9E75"}`,
                        color: forgeResult.accepted ? "#A32D2D" : "#0F6E56",
                      }}>
                        {forgeResult.reason}
                      </div>
                    )}
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      ...
                      <StatCard label="Successes" value={forgeCounts.successes}
                        accent={forgeCounts.successes > 0 ? "#A32D2D" : "#0F6E56"} />
                    </div>
              ...
              <div>
                ...
                {eufPairs ? (
                  <div style={{ border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-md)", overflow: "hidden", maxHeight: 380, overflowY: "auto" }}>
                    <div style={{ display: "grid", gridTemplateColumns: "44px 1fr 1fr", background: "var(--color-background-secondary)", borderBottom: "0.5px solid var(--color-border-tertiary)", position: "sticky", top: 0 }}>
                      {["#", "m_i (binary)", "t_i = Mac_k(m_i)"].map((h, i) => (
                        <div key={i} style={{ padding: "5px 10px", fontSize: 10, fontWeight: 500, color: "var(--color-text-secondary)", textTransform: "uppercase" }}>{h}</div>
                      ))}
                    </div>
                    {eufPairs.map((p, i) => {
                      const isForge = p.m === forgeMsg.replace(/[^01]/g, "").padEnd(8, "0").slice(0, 8);
                      return (
                        <div key={i} style={{ display: "grid", gridTemplateColumns: "44px 1fr 1fr", borderBottom: "0.5px solid var(--color-border-tertiary)", background: isForge ? "#FEF3E2" : "transparent" }}>
                          <div style={{ padding: "5px 10px", fontSize: 11, color: "var(--color-text-secondary)" }}>{i + 1}</div>
                          {/* FIX: m is binary — no 0x prefix */}
                          <div style={{ padding: "5px 10px", fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--color-text-primary)" }}>{p.m}</div>
                  ...
                  <div style={{ padding: 24, textAlign: "center", fontSize: 12, color: "var(--color-text-secondary)", fontStyle: "italic", background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)" }}>
                    Click "Generate 50 signed pairs" to start the game.
                  </div>
                )}
              </div>
            </div>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "260px minmax(0,1fr)", gap: 16 }}>
              <div>
                <div style={{ marginBottom: 10 }}>
                  <FieldLabel>Key k (hex)</FieldLabel>
                  <TextInput value={distKey} onChange={setDistKey} placeholder="b4e7a2f1" />
                </div>
                ...
                {distResult && (
                  <div style={{ marginTop: 12 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
                      <StatCard label="Total queries" value={distResult.totalQ} />
                    ...
                    <StatCard label="Mean byte diff" value={distResult.diff}
                      accent={parseFloat(distResult.diff) < 20 ? "#0F6E56" : "#A32D2D"} />
                    <div style={{ marginTop: 8, padding: "10px 12px", borderRadius: "var(--border-radius-md)", fontSize: 12,
                      background: parseFloat(distResult.diff) < 20 ? "#E1F5EE" : "#FAEEDA",
                      border: `0.5px solid ${parseFloat(distResult.diff) < 20 ? "#1D9E75" : "#BA7517"}`,
                      color: parseFloat(distResult.diff) < 20 ? "#0F6E56" : "#854F0B",
                    }}>
              ...
              {distResult && (
                <div>
                  <SectionHeading>First 10 queries — x ∈ &#x7B;0,1&#x7D;⁸</SectionHeading>
                  <div style={{ border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-md)", overflow: "hidden" }}>
                    <div style={{ display: "grid", gridTemplateColumns: "100px 1fr 1fr 60px", background: "var(--color-background-secondary)", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
                      {["x (binary)", "MAC_k(x)", "random(x)", "match?"].map((h, i) => (
                    ...
                    {distResult.queries.map((q, i) => (
                      <div key={i} style={{ display: "grid", gridTemplateColumns: "100px 1fr 1fr 60px", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
                        <div style={{ padding: "6px 10px", fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--color-text-secondary)" }}>{q.x}</div>
        ...
        {activeTab === "len-ext" && (
          <div>
            ...
            <div style={{ padding: "10px 12px", background: "#FAEEDA", borderRadius: "var(--border-radius-md)", fontSize: 12, color: "#7A4A00", marginBottom: 16, border: "0.5px solid #E8B96A", lineHeight: 1.6 }}>
              <strong>Vulnerability:</strong> If t = H(k ‖ m), an attacker who knows t and |k|
              can compute a valid tag for m ‖ pad ‖ m′ <em>without knowing k</em>,
              by resuming the hash from state t. This motivates HMAC's double-hash structure.
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "260px minmax(0,1fr)", gap: 16 }}>
              <div>
                <div style={{ marginBottom: 8 }}>
                  <FieldLabel>Key k (hex)</FieldLabel>
                  <TextInput value={leKey} onChange={setLeKey} placeholder="a3f2c1b8" />
                </div>
                <div style={{ marginBottom: 8 }}>
                  <FieldLabel>Original message m (hex)</FieldLabel>
                  <TextInput value={leMsg} onChange={setLeMsg} placeholder="deadbeef" />
                </div>
                <div style={{ marginBottom: 12 }}>
                  <FieldLabel>Attacker suffix m′ (hex)</FieldLabel>
                  <TextInput value={leSuffix} onChange={setLeSuffix} placeholder="cafebabe" />
                </div>
                <button onClick={doLenExt} style={btnStyle("#FCEBEB", "#E24B4A", "#A32D2D", { width: "100%" })}>
              ...
              <div>
                {leResult ? (
                  <div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
                      ...
                      <StatCard label="Attack succeeded?" value={leResult.attackSucceeded ? "YES ✓" : "no"}
                        accent={leResult.attackSucceeded ? "#A32D2D" : "#0F6E56"} />
                    </div>
                    <div style={{ border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-md)", overflow: "hidden", marginBottom: 12 }}>
                      {[
                        { label: "Padding inside H(k‖m)",    val: `0x${leResult.padHex}`,                        c: "#854F0B" },
                        { label: "Extended msg m‖pad‖m′",    val: `0x${leResult.extMsg.slice(0, 32)}…`,           c: "var(--color-text-primary)" },
                        { label: "Forged tag (from state)",   val: `0x${leResult.extTag}`,                        c: "#A32D2D" },
                        { label: "True H(k‖m_ext)",           val: `0x${leResult.extTagTrue}`,                    c: "#0F6E56" },
                      ].map((row, i) => (
                        <div key={i} style={{ display: "grid", gridTemplateColumns: "190px 1fr", padding: "7px 12px", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
                    ...
                    <div style={{
                      padding: "10px 14px", borderRadius: "var(--border-radius-md)", fontSize: 12, lineHeight: 1.6,
                      background: leResult.attackSucceeded ? "#FCEBEB" : "#E1F5EE",
                      border: `0.5px solid ${leResult.attackSucceeded ? "#E24B4A" : "#1D9E75"}`,
                      color: leResult.attackSucceeded ? "#A32D2D" : "#0F6E56",
                    }}>
                      {leResult.attackSucceeded
                        ? "Attack succeeded — forged tag matches H(k ‖ m_ext) without knowing k. HMAC's double-hash prevents this."
                ...
                {/* ════════════════════════════════════════════════════
            TAB 6 — HMAC stub (forward pointer to PA#10)
        ════════════════════════════════════════════════════ */}
        {activeTab === "hmac" && (
          <div>
            <SectionHeading>Construction 3 — HMAC (forward pointer: full implementation in PA#10)</SectionHeading>
 
            {/* Formula box */}
            <div style={{ padding: "14px 16px", background: "#1a1a2e", borderRadius: "var(--border-radius-md)", marginBottom: 16, border: "0.5px solid #7F77DD" }}>
              <div style={{ fontSize: 10, color: "#7F77DD", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 10 }}>HMAC Formula</div>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: 13, color: "#c8c4f8", lineHeight: 2 }}>
                HMAC<sub style={{ fontSize: 10 }}>k</sub>(m) = H( (k ⊕ opad) ‖ H( (k ⊕ ipad) ‖ m ) )
              </div>
              <div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {[
                  { term: "ipad", desc: "0x36 repeated — inner padding constant" },
                  { term: "opad", desc: "0x5C repeated — outer padding constant" },
                  { term: "H(·)",  desc: "hash function (e.g. SHA-256) — implemented in PA#10" },
                  { term: "‖",     desc: "concatenation of byte strings" },
                ].map((r, i) => (
                  <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                    <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "#7F77DD", minWidth: 40 }}>{r.term}</span>
                    <span style={{ fontSize: 11, color: "#888780" }}>{r.desc}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Why HMAC */}
            <div style={{ padding: "12px 14px", background: "#FAEEDA", borderRadius: "var(--border-radius-md)", marginBottom: 16, border: "0.5px solid #E8B96A", fontSize: 12, color: "#7A4A00", lineHeight: 1.7 }}>
              <strong>Why HMAC?</strong> The naive MAC t = H(k ‖ m) is vulnerable to length-extension attacks
              (see the Length-ext tab). HMAC defeats this by hashing <em>twice</em> with different key pads,
              so an attacker who knows H(k ‖ m) cannot continue the hash without knowing k.
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              ...
              {/* Stub demo */}
              <div>
                <div style={{ fontWeight: 500, fontSize: 12, marginBottom: 10 }}>Stub — call hmac(k, m)</div>
                <div style={{ marginBottom: 8 }}>
                  <FieldLabel>Key k (hex)</FieldLabel>
                  <TextInput value={hmacKey} onChange={setHmacKey} placeholder="a3f2c1b8" />
                </div>
                <div style={{ marginBottom: 12 }}>
                  <FieldLabel>Message m (binary string)</FieldLabel>
                  <TextInput
                    value={hmacMsg}
                    onChange={v => setHmacMsg(v.replace(/[^01]/g, ""))}
                    placeholder="e.g. 10110010"
                  />
                </div>
                <button onClick={doHmac} style={btnStyle("#EEEDFE", "#7F77DD", "#3C3489", { width: "100%" })}>
                  Call hmac(k, m)
                </button>
 
                {hmacCalled && (
                  <div style={{ marginTop: 12, padding: "12px 14px", borderRadius: "var(--border-radius-md)", background: "#FCEBEB", border: "0.5px solid #E24B4A" }}>
                    <div style={{ fontSize: 10, color: "#A32D2D", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 6 }}>
                      Exception thrown
                    </div>
                    <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "#A32D2D", fontWeight: 500 }}>
                      {hmacError}
                    </div>
                    <div style={{ marginTop: 8, fontSize: 11, color: "var(--color-text-secondary)" }}>
                      This is expected — stub raises <span style={{ fontFamily: "var(--font-mono)" }}>NotImplemented</span> until PA#10.
                    </div>
                  </div>
                )}
              </div>
              ...
              {/* Source code display */}
              <div>
                <div style={{ fontWeight: 500, fontSize: 12, marginBottom: 10 }}>Stub source (crypto_PA5.js)</div>
                <div style={{ background: "#0d1117", borderRadius: "var(--border-radius-md)", border: "0.5px solid #30363d", overflow: "hidden" }}>
                  <div style={{ padding: "6px 12px", background: "#161b22", borderBottom: "0.5px solid #30363d", fontSize: 10, color: "#8b949e", fontFamily: "var(--font-mono)" }}>
                    crypto_PA5.js — hmac stub
                  </div>
                  <div style={{ padding: "14px 16px", fontFamily: "var(--font-mono)", fontSize: 11, lineHeight: 1.9 }}>
                    {[
                      { t: "comment", s: "/**" },
                      { t: "comment", s: " * HMAC_k(m) = H( (k ⊕ opad) ‖ H( (k ⊕ ipad) ‖ m ) )" },
                      { t: "comment", s: " * Full implementation deferred to PA#10." },
                      { t: "comment", s: " * Do not use a library HMAC here or in PA#10." },
                      { t: "comment", s: " */" },
                      { t: "keyword", s: "export function ", extra: { color: "#ff7b72" }, rest: "hmac(_keyHex, _msgBin) {", restColor: "#c9d1d9" },
                      { t: "body",    s: '  throw new Error(', extra: { color: "#c9d1d9" }, rest: '"NotImplemented —', restColor: "#a5d6ff" },
                      { t: "body2",   s: '    HMAC full implementation belongs in PA#10.");', extra: { color: "#a5d6ff" } },
                      { t: "close",   s: "}" },
                    ].map((line, i) => (
                      <div key={i} style={{ whiteSpace: "pre" }}>
                        {line.t === "keyword"
                          ? <><span style={{ color: "#ff7b72" }}>{line.s}</span><span style={{ color: "#c9d1d9" }}>{line.rest}</span></>
                          : line.t === "body"
                          ? <><span style={{ color: "#c9d1d9" }}>{line.s}</span><span style={{ color: "#a5d6ff" }}>{line.rest}</span></>
                          : line.t === "body2"
                          ? <span style={{ color: "#a5d6ff" }}>{line.s}</span>
                          : <span style={{ color: line.t === "comment" ? "#8b949e" : "#c9d1d9" }}>{line.s}</span>
                        }
                      </div>
                    ))}
                  </div>
                ...
                <div style={{ marginTop: 12, padding: "10px 14px", background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", border: "0.5px solid var(--color-border-tertiary)" }}>
                  <div style={{ fontSize: 10, color: "var(--color-text-secondary)", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 8 }}>PA#10 will implement</div>
                  {[
                    "Custom H(·) hash function (no library)",
                    "ipad / opad key expansion",
                    "Inner hash: H( (k ⊕ ipad) ‖ m )",
                    "Outer hash: H( (k ⊕ opad) ‖ inner )",
                    "Vrfy_k(m, t) → bool using HMAC",
                  ].map((item, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                      <span style={{ fontSize: 10, color: "#7F77DD" }}>○</span>
